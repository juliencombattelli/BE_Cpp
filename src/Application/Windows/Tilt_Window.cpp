...
Tilt_Window::Tilt_Window(Picaso::Serial_Commander& lcd) :
	Window_Template(lcd),
...
void Tilt_Window::area_touched_handler(Sender& s, const Event& e)
{
	Picaso::Touchable_Area& sender = static_cast<decltype(sender)>(s);
	const Picaso::Area_Touched& area = static_cast<decltype(area)>(e);

	unsigned int id = sender.get_id();

	if(id == m_area.get_id())
		raise(Tilt_Change(area.x, area.y));
}
